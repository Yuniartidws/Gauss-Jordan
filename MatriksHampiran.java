...
public class MatriksHampiran {
  ...
  public static double fx ( double x){
    double ee = Math.exp(1.0);
    ee = Math.pow(ee,-x);
    return ee/(1+Math.sqrt(x)+ Math.pow(x,2));
  }
  ...
  public static void inputMatriks() {
    ...
    if (!isFileInput.equals("y") && !isFileInput.equals("Y")) System.out.print("Masukan jumlah derajat persamaan : ");
    int n = in.nextInt();
    mat = new Matriks(n+1, n+2);
    
    if (!isFileInput.equals("y") && !isFileInput.equals("Y")) System.out.print("Masukan jumlah selang hampiran : ");
    double a,b;
    a = in.nextDouble();
    b = in.nextDouble();      
    ...
    for (int i = 0; i < mat.getNRow(); i++) {  
      ...
      for (int j = 0; j < mat.getNCol()-1; j++)
        mat.setMatElement(i, j, Math.pow(x,j));

      mat.setMatElement(i, mat.getNCol()-1, fx(x));
